...
public class AnimandoCaptura
{
        ...
        switch (fase)
        {
            case FaseDoAnimaCaptura.inicial:
                ...
                if (arredondado >= LOOPS)
                {
                    if (iraCapturar)
                    {
                        EventAgregator.Publish(EventKey.stopTheMusic, null);
    ...
    {
        EventAgregator.Publish(new StandardSendStringEvent(null, SoundEffectID.encontro.ToString(), EventKey.disparaSom));
